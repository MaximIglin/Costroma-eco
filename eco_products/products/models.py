...
class Category(models.Model):
    ...
    image_url = models.URLField(
        "Ссылка на изображение", max_length=500, blank=True, null=True)
    image = models.ImageField(
        upload_to="category", verbose_name="Изображение", blank=True, null=True)

    def save(self, *args, **kwargs):

        if self.image_url and not self.image:
            temporary_image = NamedTemporaryFile(delete=True)
            temporary_image.write(
                urllib.request.urlopen(self.image_url).read())
...
class Product(models.Model):
    ...
    mass = models.DecimalField(max_digits=10,  decimal_places=2, verbose_name="Масса",
                               null=True, blank=True, validators=[MinValueValidator(0)])
    volume = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Объём",
                                 null=True, blank=True, validators=[MinValueValidator(0)])
    price = models.DecimalField(max_digits=10, decimal_places=2,
                                verbose_name="Цена", validators=[MinValueValidator(0)])
    sale = models.DecimalField(max_digits=10, decimal_places=2, default=0,
                               verbose_name="Скидка", validators=[MinValueValidator(0)])
    image = models.ImageField(
